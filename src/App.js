...
const App = () => {
	...
	const handleRuleSelect = (e, ruleSelector) => {
		if (rules) {
			e.metaKey || e.ctrlKey
				? setSelectedRules([...selectedRules, ruleSelector])
				: setSelectedRules([ruleSelector]);
		}
	};
	...
	return (
		<>
			...
			<div className="floatful--properties">
				{rules.map((rule) => {
					return (
						selectedRules.indexOf(rule.selector) >= 0 && (
							<PropertiesList
								key={rule.selector}
								rule={rule}
								dispatch={dispatch}
